...
- (RTCVideoTrack *)createFrameCaptureVideoTrack  {
    ...
    FrameCapturer *frameCapturer = [[FrameCapturer alloc] initWithDelegate:videoSource]; // constraints go here
    ...
    TrackCapturerEventsEmitter *emitter = [[TrackCapturerEventsEmitter alloc] initWith:trackUUID webRTCModule:self];
    frameCaptureController.eventsDelegate = emitter;
...
RCT_EXPORT_METHOD(getInputMedia 
                  ...
                  : (RCTResponseSenderBlock)successCallback rejector
                  : (RCTResponseSenderBlock)errorCallback) {
#if TARGET_OS_TV
    if (constraints[@"audio"]) {
        errorCallback(@[@"unsupported_platform", @"tvOS is not supported"]);
        return;
    }
#endif
